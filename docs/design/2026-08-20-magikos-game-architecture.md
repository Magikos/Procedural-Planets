...
| B14 | Six of seven `[BurstCompile]` sites do not pin `FloatMode`; two of them derive gameplay truth | `NoiseFilters/NoiseFilterData.cs:47`, `Biomes/BiomeLookupData.cs:44` | Cross-machine bit-identity is unasserted. ~~One attribute each.~~ **CORRECTED 2026-08-25 — the two sites named in the middle column are the wrong ones, and the fix is not one attribute each.** Burst's direct-call needs `[BurstCompile]` on **both** the class and the method; neither `NoiseFilterEvaluator.Evaluate` nor `BiomeLookupEvaluator.Resolve` carries a method-level attribute, so the class-level one is inert. Called from C# they run as plain managed IL — which is why `NoiseFilterEvaluatorGoldenTests` asserts Burst==managed bit-for-bit and passes. Called from a job they are inlined and compiled under **that job's** `FloatMode`. So the sites that actually govern terrain numerics are the jobs: `Surface/PlanetChunkMeshJob.cs:17` (mesh) and `:152` (normals), and `TerrainFace.cs:193` — all three unpinned. `ScatterGatherJob.cs:63` is already `Deterministic`. **Do not pin the jobs blind:** `BurstCompiler.Options.EnableFastMath` is **True** in this editor, so if `FloatMode.Default` resolves to fast math here, pinning changes terrain output and therefore every existing seed's world. Measure first — generate a fixed seed, checksum chunk vertices, pin, regenerate, compare. Applying it was attempted and reverted on 2026-08-25 once the inertness was found. |
