...
7. **Console setter**: read `docs/design/console-command-authoring.md` first. Keep command metadata and parsing in a domain adapter. The adapter calls a typed owner operation, which validates and updates the DTO. Declare a group and release policy. Return `ConsoleCommandResult.Fail` for expected rejection. Preserve renamed commands as aliases. Run `ConsoleRegressionTests` and test rejection through `CommandExecutor`. Never write the SO or shader globals from a command. This replaces the former same-class guidance as of 2026-09-07.
