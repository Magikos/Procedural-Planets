            ...
            #include "GrassColor.hlsl"
            #include "GrassInteractors.hlsl"
            #include "GrassDither.hlsl"

            #include "VegetationMotion.hlsl"
            #if defined(GRASS_MOTION_PASS)
            ...
            struct Varyings
            {
                ...
                float4 color : COLOR0;
                #if defined(GRASS_MOTION_PASS)
                    float4 currentCS : TEXCOORD8;
                    float4 previousCS : TEXCOORD9;
                #endif
            };
            ...
            float3 ComputeWindOffset(float3 rootWs, float3 upWs, float height, float t, uint seed, bool previous)
            {
                float3 windDir = SafeNormalize(previous ? _VegetationPreviousWind.xyz : _WindDirection, float3(1.0, 0.0, 0.0));
                ...
                float windForce = max(previous ? _VegetationPreviousWind.w : _WindSpeedMps, 0.0);
                float gameTime = previous ? _VegetationPreviousTime.y : _GameTime;
                ...
                float travelWave = sin(gameTime * waveFreq - dot(relRoot, windTangent) * 0.18);
                float gustEnvelope = lerp(0.7, 1.0, SmoothPatchNoise(relRoot, 8.0, 0.0));
                float bladeJitter = sin(gameTime * waveFreq * 2.5 + Hash01(seed ^ 0x44444444u) * 6.2831853) * 0.12;
            ...
            Varyings EvaluateGrassVertex(uint vertexID, uint instanceID, bool previous)
            {
                ...
                float viewDistance = length(cameraPosition - rootWS);
                ...
                float3 toCameraTangent = cameraPosition - rootWS;
                ...
                float3 interactorBend = SampleGrassInteractorBend(tuftRootWS, upWS, height * 0.85, previous) * (t * t);
                ...
                float3 windOffset = ComputeWindOffset(tuftRootWS, upWS, height, t, seed, previous);
            ...
            {
                Varyings output = EvaluateGrassVertex(vertexID, instanceID, false);
                #if defined(GRASS_MOTION_PASS)
                    Varyings previous = EvaluateGrassVertex(vertexID, instanceID, true);
                    output.currentCS = mul(_NonJitteredViewProjMatrix, float4(output.positionWS, 1));
                    output.previousCS = mul(_PrevViewProjMatrix, float4(previous.positionWS, 1));
                #endif
                return output;
            }
            ...
            half4 GrassFragment(Varyings input) : SV_Target
            {
                ...
                #if defined(GRASS_MOTION_PASS)
                    return float4(CalcNdcMotionVectorFromCsPositions(input.currentCS, input.previousCS), 0, 0);
                #else
                ...
                surfaceDirect *= MainLightShadow(TransformWorldToShadowCoord(input.positionWS), input.positionWS,
