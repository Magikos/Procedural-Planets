...
- [Ocean scatter — SHIPPED](project_ocean_scatter.md) — DO NOT repeat "scatter cannot place below the waterline"; it is FALSE. Altitude is signed, so a depth band is an ordinary altitude gate. 10 reef props as of 2026-09-03: a BRANCHING form keys almost no impostor card (cov 0.0009-0.0044), a SOLID one does (0.03, IoU 0.95). Cull 120 is a card-range cliff, not a slope.
- [Ocean wave approach](project_ocean_wave_approach.md) — Displace the existing mesh, NOT a camera-following patch.
- [Scatter LOD + impostor](project_scatter_lod_impostor.md) — Historical silhouette, lighting, and atlas investigations. Read the 2026-09-09 current-code correction before reusing bake settings; use pp-scatter-and-impostors for current procedures.
